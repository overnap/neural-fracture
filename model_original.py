...
class Model(nn.Module):
    def __init__(self, device="cpu"):
        ...
        self.fp3 = PointNetFeaturePropagation(
            in_channel=1024 + 256 + 256, mlp=[256, 256]
        )
        self.fp2 = PointNetFeaturePropagation(
            in_channel=256 + 128 + 128 + 64, mlp=[256, 128]
        )
    ...
    def forward(self, xyz, parts_count):
        ...
        parts_embedding = (
            ...
            .unsqueeze(1)
            .unsqueeze(1)
            .repeat(1, 1, l0_points.shape[2])
        )
        l0_points = self.fp1(
            l0_xyz, l1_xyz, torch.cat((l0_points, parts_embedding), dim=1), l1_points
        )

        x = l0_points
        x = torch.einsum(
            "bci,bcj->bij", x, x
        )  # inner product; get pointwise unnormalized similarity (-inf, inf)
        x = torch.sigmoid(
            x
        )  # map inner product (unnormalized similarity) to probability (0, 1)
        ...
        mask = (
            (1 - mask.cumsum(dim=1)[:, : x.shape[1]])
            .repeat(x.shape[2], 1, 1)
            .permute(1, 2, 0)
        )
        x = (torch.exp(x) * mask).permute(0, 2, 1)  # masking parts
        x = x / (
            x.sum(dim=-1, keepdim=True) + 1e-5
        )  # reducted similarity matrix, same as label probability
        return sim, x
